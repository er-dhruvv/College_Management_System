        ...
        <div className="bg-white dark:bg-slate-800 w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden">
          ...
          opacity-0 group-hover:opacity-100 cursor-pointer transition duration-300 z-10"
              ...
              <div className="flex flex-col justify-center">
                <h2 className="text-3xl font-extrabold tracking-tight drop-shadow-sm">{userInfo.fullname}</h2>
                <p className="text-indigo-100 font-medium mt-0.5 text-lg">@{userInfo.username}</p>
                <div className="mt-3">
                  <span className="inline-block text-sm bg-white/20 text-white backdrop-blur-md border border-white/30 px-4 py-1.5 rounded-full shadow-sm font-semibold tracking-wide">
                    Enrollment No: {userInfo.Enrollno || "N/A"}
                  </span>
                </div>
              </div>
          ...
          <div className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700 dark:text-gray-200">
            <ProfileItem label="Roll No" value={userInfo.rollno} />
            <ProfileItem
              label="Date of Birth"
...
const ProfileItem = ({ label, value }) => (
  <div className="bg-gray-50 border border-gray-100 dark:border-slate-700 dark:bg-slate-900 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
    <p className="text-xs text-indigo-500 dark:text-indigo-400 font-bold uppercase tracking-wider mb-1">{label}</p>
    <p className="font-semibold text-gray-800 dark:text-gray-100 break-words text-lg">{value || "N/A"}</p>
  </div>
);
